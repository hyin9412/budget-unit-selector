...
import { getNodeCheckState, searchBudgetUnits } from '@/utils/budgetHelpers'
...
function getLevelLabel(level: BudgetUnitNode['level']) {
  if (level === 1) {
    return '一级选项'
  }
...
export default function BudgetUnitSelector({
  ...
  const leftSelectableNodes = useMemo(
    ...
    [filterToTopLevel, hasSearch, matchesKeyword, searchResults, topLevelNodes, visibleNodes],
  )
  ...
  const duplicatedSelectedNames = useMemo(() => {
                    ...
                    {selectedNodes.map((node) => (
                      <div key={node.id} className="flex items-center gap-3 rounded px-3 py-2 text-[13px] leading-[22px] text-slate-700 hover:bg-slate-50">
                        <div className="min-w-0 flex-1">
                          ...
                          {duplicatedSelectedNames.has(node.name) ? (
                            <div className="truncate text-[13px] leading-[22px] text-slate-400">{getLevelLabel(node.level)}</div>
                          ) : null}
                        </div>
                        <button
                          type="button"
